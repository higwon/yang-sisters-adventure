...
function ScheduleWorkspace({ tripDates: dates, dashboard, items, planning, reload, reloadPlanning }: { tripDates: string[]; dashboard: Dashboard; items: ScheduleItem[]; planning: import('../../domain').PlanningItem[]; reload: () => void; reloadPlanning: () => void }) {
  ...
  return <section className="scheduleWorkspace">
    ...
    <div className="scheduleDesktopLayout"><div className="desktopItinerary" style={{ '--days': dates.length } as CSSProperties}>
...
function ScheduleEditor({ draft, dates, dashboard, close, reload }: { draft: Draft; dates: string[]; dashboard: Dashboard; close: () => void; reload: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setSaving(true); setError(''); const values = Object.fromEntries(new FormData(event.currentTarget)); const payload = { title: values.title, day_date: values.day_date, start_time: values.start_time || null, end_time: values.end_time || null, place_id: item?.place_id ?? null, category: values.category, notes: values.notes || null, url: values.url || null, status: values.status, participant_ids: participants, ...(item ? { sort_order: item.sort_order } : {}) }; try { if (item) await api.update('schedule', item.id, payload); else await api.create('schedule', payload); close(); reload(); } catch (reason) { setError(reason instanceof Error ? reason.message : '저장하지 못했습니다.'); setSaving(false); } };
  return <div className="backdrop" onMouseDown={close}><section className="modal scheduleEditor" onMouseDown={(event) => event.stopPropagation()}><header><div><h2>{item ? item.title : '일정 추가'}</h2></div><button onClick={close}>×</button></header><form onSubmit={submit}><label>제목<input name="title" defaultValue={item?.title} required autoFocus /></label><div className="formRow"><label>날짜<select name="day_date" defaultValue={item?.day_date ?? draft.dayDate}>{dates.map((date, index) => <option value={date} key={date}>DAY {index + 1} · {formatDay(date)}</option>)}</select></label><label>상태<select name="status" defaultValue={item?.status ?? 'confirmed'}><option value="confirmed">확정</option><option value="candidate">후보</option></select></label></div><div className="formRow"><label>시작<input name="start_time" type="time" defaultValue={item?.start_time ?? draft.startTime} /></label><label>종료<input name="end_time" type="time" defaultValue={item?.end_time ?? ''} /></label></div><label>카테고리<select name="category" defaultValue={item?.category ?? '관광'}>{categories.map((category) => <option key={category}>{category}</option>)}</select></label>{item?.place_name && <p className="legacyPlace"><MapPin />{item.place_name}</p>}<label><Link />관련 URL<input name="url" type="url" defaultValue={item?.url ?? ''} placeholder="https://" /></label><label>메모<textarea name="notes" defaultValue={item?.notes ?? ''} rows={3} /></label><fieldset><legend><Users /> 참여 멤버</legend>{dashboard.members.map((member) => <label className="check" key={member.id}><input type="checkbox" checked={participants.includes(member.id)} onChange={(event) => setParticipants(event.target.checked ? [...participants, member.id] : participants.filter((id) => id !== member.id))} /><UserAvatar user={member} />{member.name}</label>)}</fieldset>{error && <p className="errorText">{error}</p>}<footer>{item && <button type="button" className="editorDelete" onClick={async () => { if (confirm('이 일정을 삭제할까요?')) { await api.remove('schedule', item.id); close(); reload(); } }}><Trash2 />삭제</button>}<button className="primary" disabled={saving}>{saving ? '저장 중…' : '저장'}</button></footer></form></section></div>;
}
