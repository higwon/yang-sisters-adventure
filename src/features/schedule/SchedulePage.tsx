...
export function SchedulePage() {
  const trip = useTrip();
  return <Load fn={api.dashboard}>{(dashboard) => <Load fn={api.places}>{(places) => <Load fn={api.schedule}>{(items, reload) => <Load fn={api.planning}>{(planning, reloadPlanning) =>
    <ScheduleWorkspace tripDates={tripDates(trip.start_date, trip.end_date)} dashboard={dashboard} places={places} items={items} planning={planning} reload={reload} reloadPlanning={reloadPlanning} />
  }</Load>}</Load>}</Load>}</Load>;
}

function ScheduleWorkspace({ tripDates: dates, dashboard, places, items, planning, reload, reloadPlanning }: { tripDates: string[]; dashboard: Dashboard; places: Place[]; items: ScheduleItem[]; planning: import('../../domain').PlanningItem[]; reload: () => void; reloadPlanning: () => void }) {
  ...
  return <section className="scheduleWorkspace">
    ...
    </div><PlanningInbox items={planning} places={places} dates={dates} reloadPlanning={reloadPlanning} reloadSchedule={reload} /></div>
...
function ScheduleEditor({ draft, dates, places, dashboard, close, reload }: { draft: Draft; dates: string[]; places: Place[]; dashboard: Dashboard; close: () => void; reload: () => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setSaving(true); setError(''); const values = Object.fromEntries(new FormData(event.currentTarget)); const payload = { title: values.title, day_date: values.day_date, start_time: values.start_time || null, end_time: values.end_time || null, place_id: values.place_id ? Number(values.place_id) : null, category: values.category, notes: values.notes || null, url: values.url || null, status: values.status, participant_ids: participants, ...(item ? { sort_order: item.sort_order } : {}) }; try { if (item) await api.update('schedule', item.id, payload); else await api.create('schedule', payload); close(); reload(); } catch (reason) { setError(reason instanceof Error ? reason.message : '저장하지 못했습니다.'); setSaving(false); } };
