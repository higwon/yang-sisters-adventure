...
const expenseTotal = (amount: number, currency: string) => new Intl.NumberFormat('ko-KR', { style: 'currency', currency }).format(amount / (currency === 'PHP' ? 100 : 1));
...
export function HomePage() {
  ...
  return <div className="collabHome">
    ...
    <aside className="homeSide"><section><header><CheckCircle2 /><b>여행 준비</b><strong>{Math.round(progress)}%</strong></header><span className="homeProgress"><i style={{ width: `${progress}%` }} /></span><small>{data.checklist.completed}개 완료 · {data.checklist.total - data.checklist.completed}개 남음</small></section><section className="quickFacts"><p><Inbox /><span>일정 후보</span><b>{data.planningCount}개</b></p><p><WalletCards /><span>여행 비용</span><b>{data.expenseTotals.length ? data.expenseTotals.map((total) => expenseTotal(total.amount_minor, total.currency)).join(' · ') : '없음'}</b></p></section></aside></div>
