...
export function SettingsPage() {
  ...
  return <section className="settingsPage"><header><h1>설정</h1><p>프로필과 이 여행에 필요한 정보만 관리해요.</p></header>{message && <p className="settingsMessage">{message}</p>}
    ...
    <section><h2>여행 설정</h2><form onSubmit={updateTrip}><label>여행 이름<input name="name" defaultValue={trip.name} required disabled={!owner} /></label><label>목적지<input name="destination" defaultValue={trip.destination} required disabled={!owner} /></label><input type="hidden" name="country_code" value={trip.country_code} /><input type="hidden" name="timezone" value={trip.timezone} /><p className="settingDates">여행 날짜 · {trip.start_date.replaceAll('-', '.')} — {trip.end_date.replaceAll('-', '.')}</p>{owner && <button className="primary">여행 설정 저장</button>}</form></section>
