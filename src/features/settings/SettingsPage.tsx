...
export function SettingsPage() {
  ...
  return <section className="settingsPage"><header><h1>설정</h1><p>프로필과 이 여행에 필요한 정보만 관리해요.</p></header>{message && <p className="settingsMessage">{message}</p>}{error && <p className="errorText">{error}</p>}
    <section><h2>내 프로필</h2><div className="profileSettings"><UserAvatar user={user} className="largeAvatar" /><div><label className="imageButton"><Camera />사진 변경<input type="file" accept="image/jpeg,image/png,image/webp" onChange={async (event) => { const file = event.target.files?.[0]; if (file) { try { await api.uploadAvatar(file); await refreshUser(); } catch (reason) { setError(reason instanceof Error ? reason.message : '사진을 저장하지 못했어요.'); } } }} /></label>{user.avatar_key && <button className="quietDanger" onClick={async () => { await api.removeAvatar(); await refreshUser(); }}>사진 삭제</button>}<small>JPEG, PNG, WEBP · 최대 2MB</small></div></div><form onSubmit={updateProfile}><label>이름<input name="name" defaultValue={user.name} maxLength={30} required /></label><button className="primary">내 프로필 저장</button></form></section>
