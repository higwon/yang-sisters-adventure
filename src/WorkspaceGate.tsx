...
function CreateTrip({ close, done }: { close: () => void; done: (trip: TripSummary) => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setError(''); const values = Object.fromEntries(new FormData(event.currentTarget)); try { done((await api.createTrip({ name: String(values.name), destination: String(values.destination), country_code: 'XX', start_date: String(values.start_date), end_date: String(values.end_date), timezone: Intl.DateTimeFormat().resolvedOptions().timeZone })).trip); } catch (reason) { setError(reason instanceof Error ? reason.message : '여행을 만들지 못했습니다.'); } };
