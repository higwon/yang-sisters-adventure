...
export function WorkspaceGate({ children }: { children: ReactNode }) {
  ...
  const removeCurrentTrip = () => { const removedId = selected?.id; setTrips((current) => current?.filter((trip) => trip.id !== removedId)); setSelected(undefined); api.setTrip(null); navigateToTrips(true); };
  useEffect(() => { reload(); }, []);
  ...
  if (selected) return <TripProvider value={selected} updateCurrentTrip={updateCurrentTrip} removeCurrentTrip={removeCurrentTrip}>{children}</TripProvider>;
