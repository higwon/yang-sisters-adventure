...
planningRoutes.post('/planning/:id/schedule',zValidator('json',convertSchema),async(c)=>{
  ...
  const endNextDay=Boolean(data.start_time&&data.end_time&&data.end_time<data.start_time);
  const schedule=await c.env.DB.prepare(`INSERT INTO schedule_items(trip_id,trip_day_id,place_id,title,start_time,end_time,end_next_day,category,notes,status,url,sort_order) VALUES(?,?,?,?,?,?,?,?,?,?,?,?) RETURNING *`).bind(tripId,day.id,item.place_id,item.title,data.start_time??null,data.end_time??null,endNextDay?1:0,item.item_type,item.notes,'candidate',item.url,order).first<{id:number}>();
